...
import { FFmpegJob, FFmpegJobOptions } from '../../jobs/ffmpeg.js';
import { YTdlpJob, YTdlpJobOptions } from '../../jobs/ytdlp.js';
...
export class Program {
    ...
    constructor() {
        ...
        this.program
            .name('v10m')
            .description('A versatile media downloader with built-in compression.')
            .version(this.configs.config.version + (process.env.V10M_DEV ? ` (debug)` : ''))
        ...
        this.program
            ...
            .option('-kb, --filesize-kilobytes <kb>', 'target file size in kilobytes', parseInt)
            .action((input, options) => {
                options.output = options.output.replace('%(input)s', input.slice(input.lastIndexOf('/') + 1, input.lastIndexOf('.')));
