  ...
  start() {
    ...
    nvidia.on('error', (_) => { /* Ignore errors */ }).on('close', (nvidiaExit) => {
      ...
      t.stderr.on('data', (data) => {
        ...
        const durationSeconds = parseInt(durationString[1], 10) * 3600 + parseInt(durationString[2], 10) * 60 + parseFloat(durationString[3]);
        const maxSizeKilobits = (this.options.fileSizeKilobytes ?? fs.statSync(this.inputFile).size / 1024) * 8;
        const bitrate = ((maxSizeKilobits / durationSeconds) - FFMPEG_AUDIO_BITRATE) * (1 - FFMPEG_VIDEO_BITRATE_BUFFER);
