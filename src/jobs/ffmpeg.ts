...
/**
 * Class representing a transcode job.
 */
export class FFmpegJob extends Job {
    ...
    start() {
        ...
        this.ffmpeg.on('error', (error) => {
            if (this.handle('ffmpeg::error', error, true)) return;
        });

        this.ffmpeg.on('close', (code) => {
            if (this.handle('ffmpeg::close', code, code !== 0)) return;
        ...
        this.ffmpeg.stderr.on('data', (data) => {
            ...
            if (this.handle('ffmpeg::out', data, /error[:|\]]/mi.test(data))) return;
